...
A second set lives in `.agents/skills/`, installed with `npx skills add Leonxlnx/taste-skill` ([Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill)) and symlinked into `.claude/skills/`: `design-taste-frontend`, `high-end-visual-design`, `redesign-existing-projects`, `minimalist-ui`, `industrial-brutalist-ui`, `brandkit`, `gpt-taste`, `stitch-design-taste`, `image-to-code`, `imagegen-frontend-web`/`-mobile`, `full-output-enforcement`. Re-run that command to update them; edit the files under `.agents/skills/`, never the symlinks.
